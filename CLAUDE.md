...
- **Image inlining**: `SeleniumCrawler._inline_images_as_base64()` converts all `<img src="http...">` to base64 data URIs via browser `fetch()` before extracting `page_source`. This uses the browser's authenticated session to bypass Vercel's image blocking. `ContentParser.process_images` skips `data:` URIs in URL normalization.
...
- **NotebookLM upload**: After successful release, Korean PDFs are uploaded to NotebookLM via `notebooklm-py` (unofficial API). Date-stamped filenames, 2 versions kept per type (previous + current) for diff queries. Requires `NOTEBOOKLM_STORAGE_STATE` (base64 Playwright session) and `NOTEBOOKLM_NOTEBOOK_ID` secrets. Session may expire and require manual `notebooklm login` + secret refresh.
